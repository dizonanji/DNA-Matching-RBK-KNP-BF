...
# DNA / Pattern files
DNA_INPUT_FILE = "dog.txt"
DNA_PATTERN_FILE = None       # Optional: file containing pattern
DNA_PATTERN_FOR_FILE = "GCTA" # Default pattern if no file
...
def load_dna_from_file(file_path):
    """Load a DNA sequence from a file (FASTA or plain text)."""
...
def load_pattern_from_file(file_path):
    """Load a DNA pattern from a file."""
    with open(file_path, 'r') as f:
        raw = f.read().upper()
...
def run_tests():
    ...
    # 1) USER FILE (repeated REPETITIONS times)
    if USE_FILE and DNA_INPUT_FILE:
        text = load_dna_from_file(DNA_INPUT_FILE)
        pattern = load_pattern_from_file(DNA_PATTERN_FILE) if DNA_PATTERN_FILE else DNA_PATTERN_FOR_FILE
    ...
    for idx, (text, pattern, expected, desc) in enumerate(sequences):
        ...
        for run in range(REPETITIONS):
            ...
            if run == 0:
                ...
                rk_correct_vs_expected = (rk_matches == expected) if expected else None

        avg_naive = sum(naive_times)/REPETITIONS
        avg_kmp = sum(kmp_times)/REPETITIONS
        avg_rk = sum(rk_times)/REPETITIONS

        print(f"     Naive Average Time: {avg_naive:.6f}s | Matches: {num_naive}")
        print(f"       KMP Average Time: {avg_kmp:.6f}s | Matches: {num_kmp} | KMP matched to BF: {correct_kmp}")
        print(f"Rabin–Karp Average Time: {avg_rk:.6f}s | Matches: {num_rk} | RK matched to BF: {correct_rk} | Correct vs expected: {rk_correct_vs_expected}")
...
def save_results(results, filename="benchmark_results.csv"):
    with open(filename, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow([
            "Text Size", "DNA Text", "Pattern", "Description",
            "Naive Matches", "KMP Matches", "RK Matches",
            "#Naive", "#KMP", "#RK",
            "KMP Correct", "RK Correct", "KMP Diff", "RK Diff",
            "Naive Avg Time", "KMP Avg Time", "RK Avg Time",
            "Expected Matches"
        ])
...
def plot_results(results):
    ...
    def plot_group(group, title):
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=group_sizes, y=naive_times, mode='lines+markers', name='Naive', marker=dict(symbol='circle', size=8), line=dict(color='blue')))
        fig.add_trace(go.Scatter(x=group_sizes, y=kmp_times, mode='lines+markers', name='KMP', marker=dict(symbol='triangle-up', size=8), line=dict(color='green')))
        fig.add_trace(go.Scatter(x=group_sizes, y=rk_times, mode='lines+markers', name='Rabin-Karp', marker=dict(symbol='diamond', size=8), line=dict(color='red')))
        fig.update_layout(title=title, xaxis_title='DNA Sequence Length', yaxis_title='Average Execution Time (seconds)', legend_title='Algorithm', template='plotly_white', hovermode='x unified')
        fig.show()

    if USE_PREDEFINED: plot_group(predefined, "Predefined Sequence Tests")
    if USE_MANUAL: plot_group(manual_seq, "Manual Sequence Tests")
    if USE_RANDOM: plot_group(random_seq, "Randomly Generated DNA Sequence Tests")
    if USE_FILE: plot_group(user_file, f"User DNA File: {DNA_INPUT_FILE}")
...
def print_environment_info():
    ...
    if DNA_INPUT_FILE: print(f"User DNA file: {DNA_INPUT_FILE}")
    if USE_FILE and DNA_PATTERN_FILE: print(f"User pattern file: {DNA_PATTERN_FILE}")
